...
const ClassSubject = forwardRef<any, ClassSubjectProps>(
  (
    {
      initialData = [],
      onChange,
    ...
    const ukEducationLevels = [
      { value: "primary", label: "Primary (Key Stage 1 & 2)" },
      { value: "ks3", label: "Key Stage 3 (Years 7-9)" },
      { value: "gcse", label: "GCSE (Years 10-11)" },
      { value: "as-level", label: "AS Level (Year 12)" },
      { value: "a-level", label: "A Level (Year 13)" },
    ...
    const isSubjectSelected = (levelIndex: number, subject: string) => {
      return studyLevels[levelIndex].subjects.some(
        (s) => s.subject === subject
      );
    };
                      ...
                      >
                        ...
                        <div className="flex items-center space-x-2">
                          <input
                            type="number"
                            value={
                              isSelected ? selectedSubject?.price || 30 : 30
                            }
                            onChange={(e) => {
                              e.stopPropagation();
                              if (isSelected) {
                                const subjectIndex =
                                  levelData.subjects.findIndex(
                                    (s) => s.subject === subject
                                  );
                                if (subjectIndex >= 0) {
                                  handlePriceChange(
                    ...
                    <div className="flex items-center space-x-2">
                      {(customSubjects[levelIndex] || "").trim() ? (
                        <button
                          ...
                          className="px-3 py-1 bg-bgprimary text-white text-sm rounded hover:bg-bgprimary/90 transition-colors"
                        >
                          Add
                        </button>
                      ) : (
