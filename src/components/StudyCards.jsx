...
const StudyCard = (props) => {
  ...
  const teacherViewingStudents =
    userRole === "teacher" &&
    (activeTab === "your-students" || activeTab === "other-students");
  console.log(
    "StudyCard - activeTab:",
    activeTab,
    "userRole:",
  ...
  if (teacherViewingStudents) {
    ...
    console.log(
      "StudyCard - Excluding email and grade for teacher viewing students"
    );
  }
  ...
  const cardStatus = displayProps.status
    ? displayProps.status.toLowerCase()
    : null;
  const statusColors = {
  ...
  const displayImage =
    ...
    image ||
    "https://images.unsplash.com/photo-1497633762265-9d179a990aa6?w=300&h=200&fit=crop&crop=center";
    ...
    return (
      <div className="flex flex-wrap gap-1 mt-1">
        {subjectsArray.map((subject, index) => (
          <span
            key={index}
            className="px-2 py-1 bg-teal-100 text-bgprimary text-xs rounded-full font-medium"
          >
            {subject}
          </span>
        ))}
  ...
  const isAssignment =
    displayProps.heading || displayProps.assignedDate || displayProps.dueDate;
  ...
  const renderAssignmentCard = () => {
    ...
    const cardData =
      ...
      (props.data.heading || props.data.description)
        ? props.data
        : displayProps;
    ...
    return (
      <div className="bg-white border border-gray-300 rounded-2xl p-6 w-full max-w-sm shadow-md transition-transform transform hover:scale-105">
        {/* Assignment Image */}
        <img
          src={assignmentImage}
          ...
          onError={(e) => {
            e.target.src =
              "https://via.placeholder.com/400x200?text=Assignment";
          }}
        />
        {/* Assignment Header */}
    ...
    <>
      {isAssignment ? (
        ...
        <div
          ...
          {...(getButtonText() === "View Details"
            ? {}
            : { onClick: () => props.onCardClick && props.onCardClick(props) })}
        >
          {/* Profile Image */}
          <img
            src={displayImage}
            ...
            onError={(e) => {
              e.target.src =
                "https://via.placeholder.com/300x200/f0f0f0/999999?text=Profile";
            }}
          />
          <div className="space-y-3">
              ...
              <div className="flex justify-end">
                <span
                  className={`px-3 py-1 rounded-full text-sm font-medium ${
                    statusColors[cardStatus] || "bg-gray-100 text-gray-600"
                  }`}
                >
            ...
            {displayProps.timeslot && (
              <div className="flex gap-x-2 items-start">
                ...
                <div className="flex-1">
                  <p className="text-md text-gray-800">
                    {formatTimeslot(displayProps.timeslot)}
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="mt-4 flex gap-4">
            <button
              onClick={(e) => {
                e.stopPropagation();
                (getButtonText() === "Connect" && props.onConnect
                  ? props.onConnect
                  : onViewSubmission)();
              }}
              className="w-full flex items-center justify-center gap-2 border border-teal-500 text-teal-600 font-medium py-2 rounded-3xl hover:bg-teal-50 transition"
