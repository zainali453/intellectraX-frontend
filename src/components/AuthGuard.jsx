...
const AuthGuard = ({ children, requireAuth = true }) => {
  ...
  try {
    ...
    console.error(
      "❌ [AuthGuard] Error getting current user from localStorage:",
      error
    );
  ...
  if (
    ...
    console.log(
      "✅ [AuthGuard] User is fully verified and onboarded (or student)"
    );
    ...
    if (
      ...
      console.log(
        "AuthGuard: Fully verified user trying to access auth pages, redirecting to dashboard"
      );
    ...
    if (!location.pathname.startsWith("/dashboard")) {
      console.log(
        "AuthGuard: Fully verified user trying to access non-dashboard page, redirecting to dashboard"
      );
      return <Navigate to="/dashboard" replace />;
    }
  ...
  if (
    ...
    if (!userData.onboarding) {
      console.log(
        "AuthGuard: Authenticated user not onboarded, redirecting to onboarding"
      );
      return <Navigate to="/onboarding" replace />;
    } else if (userData.onboarding && userData.verified !== "verified") {
      console.log(
        "AuthGuard: User onboarded but not verified, redirecting to verification pending"
      );
  ...
  if (
    ...
    console.log(
      userData.token,
      userData.onboarding,
      "AuthGuard: User not onboarded, redirecting to onboarding"
    );
    return <Navigate to="/onboarding" replace />;
  }
