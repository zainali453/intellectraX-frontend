  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await studentService.getQuizDetails(quizId);
  ...
  const handleAttachmentChange = async (file: File | null) => {
    if (file) {
      try {
        ...
        const response = await studentService.quizSubmission(
          formDataToUpload,
          quizId || ""
        );
        ...
        <>
          ...
          <div className='mt-8 bg-white p-6 rounded-2xl shadow-sm border border-gray-100'>
            <div>
              ...
              {quizDetails.isSubmitted === false && quizDetails.attachment && (
                <CustomUpload
                  label={
                    new Date() > new Date(quizDetails.endTime)
                      ? "Quiz Time has ended"
                  ...
                  uploading={uploading}
                  error={uploadError}
                  disabled={new Date() > new Date(quizDetails.endTime)}
                />
              )}
