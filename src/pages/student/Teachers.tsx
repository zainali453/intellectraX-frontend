...
import { Teacher, studentService } from "@/services/student.service";
...
const Teachers = () => {
  ...
  useEffect(() => {
    const fetchTeachers = async () => {
      try {
        setLoading(true);
        const response = await studentService.getTeachers();
          ...
          {!loading &&
            currentStudents.map((student, index) => (
              <TeacherCard
                ...
                teacher={student}
                onClick={() =>
                  navigate("/student/teachers/" + student.teacherId)
                }
                onSendMessage={() =>
                  console.log("Send message to:", student.teacherId)
                }
              />
            ))}
        </div>
        ...
        {filteredStudents.length === 0 && !loading && (
          <div className='text-center py-12'>
            ...
            <p className='text-gray-500 text-lg font-medium mb-2'>
              {searchValue
                ? `No Teacher found matching "${searchValue}"`
                : "No Teacher assigned yet"}
            </p>
            <p className='text-gray-400 text-sm'>
              {searchValue
                ? "Try searching with different keywords"
                : "Teachers will appear here once they are assigned to you."}
            </p>
          </div>
        )}
      </div>
