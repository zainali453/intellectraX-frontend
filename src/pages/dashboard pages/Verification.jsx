...
import { CalendarDays, ChevronLeft, ChevronRight, Search, Check, Eye, X } from "lucide-react";
...
export default function VerificationsPage() {
    ...
    const fetchTeachers = async (page = 1) => {
        try {
            ...
            const response = await AuthService.getUser(status, rowsPerPage, page, 'Teacher');
    ...
    const handleVerifyClick = (teacher) => {
        ...
        setModalConfig({
            ...
            description: "Are you sure you want to verify this teacher? This action will approve their profile and allow them to start teaching.",
            buttonText: "Verify",
            status: "success",
            showIcon: true,
    ...
    const handleRejectClick = (teacher) => {
        ...
        setModalConfig({
            ...
            description: "Are you sure you want to reject this teacher? This action will deny their profile and they will need to reapply.",
            buttonText: "Reject",
            status: "error",
            showIcon: true,
    ...
    const confirmVerify = async (teacherData) => {
        ...
        try {
            ...
            const response = await AuthService.TeacherVerification(teacher.email, 'verified');
            ...
            showErrorModal(
                "Verification Failed",
                error.message || "An error occurred while verifying the teacher. Please try again."
            );
        }
    };
    ...
    const confirmReject = async (teacherData) => {
        ...
        try {
            ...
            const response = await AuthService.TeacherVerification(teacher.email, 'rejected');
            ...
            showErrorModal(
                "Rejection Failed",
                error.message || "An error occurred while rejecting the teacher. Please try again."
            );
        }
    };
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: row => (
                <span
                    className={`${row.profilePicture ? 'text-teal-600 underline cursor-pointer hover:text-teal-700' : 'text-gray-500'}`}
                    onClick={() => row.profilePicture && handleProfilePictureClick(row.profilePicture)}
                >
                    {row.govId}
                </span>
            ),
        },
        {
            ...
            cell: row => (
                <span
                    className={`${row.qualifications && row.qualifications.length > 0 ? 'text-teal-600 underline cursor-pointer hover:text-teal-700' : 'text-gray-500'}`}
                    onClick={() => row.qualifications && row.qualifications.length > 0 && handleQualificationsClick(row.qualifications)}
                >
                    {row.degree}
                </span>
            ),
                ...
                <div className="flex pb-2 justify-center w-full">
                    {["Pending", "Accepted", "Rejected"].map(status => (
                        <button
                            ...
                            className={`pb-2 flex-1 text-center border-b ${subTab === status
                    ...
                    {loading ? (
                        ...
                        <>
                            ...
                            {filteredData.length === 0 && !loading && !error && (
                                <div className="text-center py-8">
                                    <div className="text-gray-500">No teachers found for the selected criteria</div>
                                </div>
                            )}
                        </>
                    )}
