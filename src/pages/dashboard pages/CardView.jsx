...
const CardView = ({ title, data, userRole, activeTab, onTabChange }) => {
  ...
  const getFilteredData = () => {
    if (
      ...
      const currentData =
        currentActiveTab === "your-teachers"
          ? data.yourTeachers
          : data.otherTeachers;
      ...
      const currentData =
        currentActiveTab === "your-students"
          ? data.yourStudents
          : data.otherStudents;
      return currentData.filter(
        (item) =>
    ...
    if (isAssignmentOrQuiz) {
      setSelectedAssignment({
        email:
          ...
          item.teacherEmail ||
          item.assignedTo ||
          item.student, // fallback for email
  ...
  const getEditDialogMode = () => {
    const detectedMode = title?.toLowerCase().includes("assignment")
      ? "create-assignment"
      : title?.toLowerCase().includes("quiz")
      ? "create-quiz"
        ...
        <div className="flex items-center gap-2">
          ...
          {isTeacherOrAdmin && (
            <button
              onClick={() => {
                console.log(
                  "🎯 [CardView] Create button clicked for title:",
                  title
                );
                console.log("🎯 [CardView] User role:", user?.role);
                console.log(
                  "🎯 [CardView] Detected mode:",
                  getEditDialogMode()
                );
          ...
          <div className="flex border-b border-gray-200 mb-6">
            {tabOptions.map((tab) => (
              <button
                ...
                className={`w-1/2 py-3 text-center font-bold transition ${
        ...
        {tabLoading ? (
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <p className="text-gray-600">
                Loading {getTabDisplayName(currentActiveTab).toLowerCase()}...
              </p>
            </div>
          </div>
        ) : (
          <div className="flex gap-6 justify-evenly flex-wrap">
            ...
            {paginatedData.map((cls, idx) => {
              const connectProps =
                isStudentView && currentActiveTab === "other-teachers"
              ...
              if (
                isStudentView &&
                (currentActiveTab === "your-teachers" ||
                  currentActiveTab === "other-teachers")
              ) {
                role = "teacher";
              } else if (isTeacherView && title?.toLowerCase() === "students") {
                role = "student";
